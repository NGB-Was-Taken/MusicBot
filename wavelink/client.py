...
class Client:
    ...
    def __new__(cls, *args, **kwargs):
        ...
        try:
            ...
            msg = 'wavelink.Client: bot is a required keyword which is missing.'
            raise WavelinkException(msg)

        if not isinstance(bot, (commands.InteractionBot, commands.AutoShardedBot, commands.AutoShardedInteractionBot, commands.Bot)):
            msg = f'wavelink.Client expected type <commands.InteractionBot> or <commands.AutoShardedInteractionBot> or <commands.AutoShardedBot> or <commands.Bot> not {type(bot)}'
            raise TypeError(msg)

        try:
            update_handlers = bot.extra_events['on_socket_responce']
        except KeyError:
            return super().__new__(cls)

        for handler in update_handlers:
            if handler.__self__.__class__.__qualname__ == 'wavelink.Client':
                bot.remove_listener(handler, 'on_socket_responce')
